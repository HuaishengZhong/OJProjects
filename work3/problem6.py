...
Output
对每一个用例输出两个距离最近的点（坐标使用空格隔开），用逗号隔开，
先按照第一个坐标大小排列，再按照第二个坐标大小排列。如果有多个解，
则按照每个解的第一个点的坐标排序，连续输出多个解，用逗号隔开。
...
def is_nearest(dist, n1, n2):
    if dist == nodeNearest.dist:
        nodeNearest.add([n1, n2])
    elif dist < nodeNearest.dist:
        nodeNearest.clear_add([n1, n2])
        nodeNearest.dist = dist


def f(s, left, right):
    if right - left == 1:  # 仅两个点
        d = distance(s[left], s[right])
        is_nearest(d, s[left], s[right])
        return d
    if right - left == 2:  # 仅三个点
        ...
        min_d = min(d1, d2, d3)
        if d1 == min_d:
            is_nearest(d1, s[left], s[left + 1])
        if d2 == min_d:
            is_nearest(d1, s[left + 1], s[right])
        if d3 == min_d:
            is_nearest(d1, s[left], s[right])
        return min_d
            ...
            if j < len(inner_s):
                if inner_s[j].y - inner_s[i].y >= dist:
                    ...
                    if d <= dist:
                        ...
                        is_nearest(d, inner_s[j], inner_s[i])
                        dist = d
            else:
                break
    return dist
